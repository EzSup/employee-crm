        ...
        IPersonSubmissionService personSubmissionService)
    {
        if (await botUser.RequestData.Peek().TryApply(message, botClient))
        {
            ...
            else
            {
                ...
                await botClient.SendTextMessageAsync(botUser.UserTgID, FORM_FILLED_SUCCESSFULLY,
                    replyMarkup: new ReplyKeyboardRemove());
            }
        }
        else
        {
            await botClient.SendTextMessageAsync(botUser.UserTgID, WRONG_DATA_ENTERED);
        }
    }
}